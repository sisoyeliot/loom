...
static int try_compiler(const char *cc, loom_toolchain_t *tc) {
  ...
  if (strstr(buf, "clang") || strstr(buf, "Clang")) {
    ...
    if (v)
      sscanf(v + 8, "%d.%d.%d", &tc->ver_major, &tc->ver_minor, &tc->ver_patch);
  } else if (strstr(buf, "gcc") || strstr(buf, "GCC")) {
    tc->type = LOOM_CC_GCC;
    char *v = strstr(buf, ") ");
    if (v)
      sscanf(v + 2, "%d.%d.%d", &tc->ver_major, &tc->ver_minor, &tc->ver_patch);
    if (tc->ver_major == 0) {
      v = strstr(buf, "version ");
      if (v)
        sscanf(v + 8, "%d.%d.%d", &tc->ver_major, &tc->ver_minor,
               &tc->ver_patch);
    }
...
int loom_detect_toolchain(loom_toolchain_t *tc) {
  ...
  cc = getenv("LOOM_CC");
  if (cc && cc[0] && try_compiler(cc, tc) == 0)
    goto found;
...

  {
    ...
    for (int i = 0; candidates[i]; i++)
      if (try_compiler(candidates[i], tc) == 0)
        goto found;
  }

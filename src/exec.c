...
int loom_nproc(void) {
  ...
  return sysinfo.dwNumberOfProcessors > 0 ? (int)sysinfo.dwNumberOfProcessors
                                          : 1;
}
...
int loom_exec_capture(char **argv, char *buf, size_t bufsz) {
  ...
  size_t n = 0;
  DWORD read_bytes;
  while (n < bufsz - 1 && ReadFile(hChildStdOutRd, buf + n,
                                   (DWORD)(bufsz - 1 - n), &read_bytes, NULL)) {
    if (read_bytes == 0)
      break;
    n += read_bytes;
  }
...
int loom_exec_jobs(loom_job_t *jobs, int n, int total, int maxj, int verbose) {
  ...
  if (maxj < 1)
    maxj = 1;
  if (maxj > MAXIMUM_WAIT_OBJECTS)
    maxj = MAXIMUM_WAIT_OBJECTS;
  if (maxj > n)
    maxj = n;

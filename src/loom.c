...
void loom_link_system_library(loom_target_t *t, const char *name) {
    ...
    if (loom_exec_capture(av2, buf, sizeof(buf)) == 0 && buf[0]) {
        ...
        char *tok = strtok_r(buf, " \t\n", &sv);
        while (tok) {
            if (tok[0]) loom_add_ldflag(t, tok);
            tok = strtok_r(NULL, " \t\n", &sv);
        }
        return;
    }
...
const char *loom_option_str(loom_build_t *b, const char *name, const char *def) {
    for (int i = 0; i < b->n_opts; i++)
        if (strcmp(b->opts[i].key, name) == 0) return b->opts[i].val;
    return def;
}
...
static const char *std_flag(loom_std_t s) {
    switch (s) {
    case LOOM_C89: return "-std=c89";
    case LOOM_C99: return "-std=c99";
    case LOOM_C11: return "-std=c11";
    case LOOM_C17: return "-std=c17";
    case LOOM_C23: return "-std=c23";
    }
    return "-std=c11";
}
...
static const char *opt_flags(loom_opt_t o) {
    switch (o) {
    case LOOM_OPT_NONE:          return "-O0";
    case LOOM_OPT_DEBUG:         return "-Og -g";
    case LOOM_OPT_RELEASE_SAFE:  return "-O2 -g";
    case LOOM_OPT_RELEASE_FAST:  return "-O3";
    case LOOM_OPT_RELEASE_SMALL: return "-Os";
    }
    return "-O0";
}
...
static char *build_flags_key(loom_build_t *b, loom_target_t *t) {
    ...
    off += snprintf(buf + off, cap - off, "%s %s %s",
                    b->tc.cc, std_flag(t->std), opt_flags(t->opt));
...
#define ARGV_PUSH(s) do {                                             \
    if (ac >= cap - 1) { cap *= 2; av = realloc(av, cap * sizeof(char *)); } \
    av[ac++] = strdup(s);                                              \
} while (0)
...
static char **make_compile_argv(loom_build_t *b, loom_target_t *t,
    ...
    if (b->release && t->opt == LOOM_OPT_NONE) {
        ...
        while (tok) { ARGV_PUSH(tok); tok = strtok_r(NULL, " ", &sv); }
        free(tmp);
    }

    if (t->type == LOOM_SHARED_LIB) ARGV_PUSH("-fPIC");
    ...
    ARGV_PUSH("-MD"); ARGV_PUSH("-MF"); ARGV_PUSH(dep);
    ARGV_PUSH("-c");
    ARGV_PUSH(src);
    ARGV_PUSH("-o");
    ARGV_PUSH(obj);
...
static char **make_link_argv(loom_build_t *b, loom_target_t *t,
                              char **objs, int n_objs, const char *out) {
    int ac = 0, cap = 64;
    char **av = calloc(cap, sizeof(char *));
    ...
    if (t->type == LOOM_STATIC_LIB) {
        ...
        ARGV_PUSH("rcs");
        ARGV_PUSH(out);
        for (int i = 0; i < n_objs; i++) ARGV_PUSH(objs[i]);
    } else {
        ARGV_PUSH(b->tc.cc);
        if (t->type == LOOM_SHARED_LIB) ARGV_PUSH("-shared");
        for (int i = 0; i < n_objs; i++) ARGV_PUSH(objs[i]);
        ARGV_PUSH("-o");
        ARGV_PUSH(out);
...
static int build_target(loom_build_t *b, loom_target_t *t) {
    ...
    free(obj_base); free(dep_base); free(cch_base);
    ...
    if (need_link) {
        ...
        if (t->outdir) loom_mkdir_p(t->outdir);
        char **lav = make_link_argv(b, t, obj_paths, nsrc, outpath);
        if (b->verbose) {
            fprintf(stderr, " ");
            for (int k = 0; lav[k]; k++) fprintf(stderr, " %s", lav[k]);
            fprintf(stderr, "\n");
        }
        rc = loom_exec(lav);
        free_argv(lav);
    ...
    for (int i = 0; i < nc; i++) { free(clabels[i]); free_argv(cjobs[i].argv); }
    free(cjobs); free(clabels);
    for (int i = 0; i < nsrc; i++) free(obj_paths[i]);
    free(obj_paths); free(flagskey);
    free(obj_dir); free(dep_dir); free(cch_dir); free(outpath);
    return rc;
}
    ...
    while (changed) {
        ...
        for (int i = 0; i < b->n_steps; i++) {
            ...
            for (int j = 0; j < s->n_deps; j++)
                if (!b->steps[s->deps[j]]->done) { ready = 0; break; }
            if (ready) {
...
static int do_query(loom_build_t *b) {
    ...
    if (tc->type == LOOM_CC_GCC)   cc_str = "gcc";
    if (tc->type == LOOM_CC_CLANG) cc_str = "clang";
    if (tc->type == LOOM_CC_TCC)   cc_str = "tcc";

    const char *os_str = "unknown";
    if (tc->os == LOOM_OS_LINUX)   os_str = "linux";
    if (tc->os == LOOM_OS_MACOS)   os_str = "macos";
    if (tc->os == LOOM_OS_FREEBSD) os_str = "freebsd";
    if (tc->os == LOOM_OS_WINDOWS) os_str = "windows";

    const char *ar_str = "unknown";
    if (tc->arch == LOOM_ARCH_X86_64)  ar_str = "x86_64";
    if (tc->arch == LOOM_ARCH_AARCH64) ar_str = "aarch64";
    if (tc->arch == LOOM_ARCH_X86)     ar_str = "x86";
    if (tc->arch == LOOM_ARCH_ARM)     ar_str = "arm";
    if (tc->arch == LOOM_ARCH_RISCV64) ar_str = "riscv64";

    printf("toolchain:\n");
    printf("  compiler : %s (%s %d.%d.%d)\n", tc->cc, cc_str,
           tc->ver_major, tc->ver_minor, tc->ver_patch);
    printf("  archiver : %s\n", tc->ar);
    printf("  os       : %s\n", os_str);
    printf("  arch     : %s\n", ar_str);
    printf("  jobs     : %d\n", b->jobs);
    ...
    for (int i = 0; i < b->n_targets; i++) {
        ...
        const char *k = "exe";
        if (t->type == LOOM_STATIC_LIB) k = "static";
        if (t->type == LOOM_SHARED_LIB) k = "shared";
        printf("  %s (%s, %d sources%s)\n", t->name, k,
               t->sources.count, t->test ? ", test" : "");
    }
    return 0;
}
...
static int do_run(loom_build_t *b) {
    for (int i = 0; i < b->n_targets; i++) {
        loom_target_t *t = b->targets[i];
        if (t->type != LOOM_EXECUTABLE || t->test) continue;
        ...
        for (int j = 0; j < b->argc; j++) {
            if (strcmp(b->argv[j], "--") == 0) { past = 1; continue; }
            if (past) rav[n++] = b->argv[j];
        }
        rav[n] = NULL;
...
static int do_test(loom_build_t *b) {
    ...
    for (int i = 0; i < b->n_targets; i++) {
        loom_target_t *t = b->targets[i];
        if (!t->test || t->type != LOOM_EXECUTABLE) continue;
...
static int do_install(loom_build_t *b) {
    ...
    for (int i = 0; i < b->n_targets; i++) {
        ...
        snprintf(cmd, sizeof(cmd), "install -m %s '%s' '%s'",
                 t->type == LOOM_EXECUTABLE ? "755" : "644",
                 src_path, dst_path);
        LOOM_INFO("install %s -> %s", name, dst_path);
        int rc = system(cmd);

        free(dst_dir); free(src_path); free(dst_path);
        if (rc != 0) return 1;
    }
    ...
    for (int i = 0; i < b->install_hdrs.count; i++) {
        ...
        free(incdir); free(dst);
        if (rc != 0) return 1;
        any = 1;
    }

    if (!any) LOOM_WARN("no targets marked for installation");
    return 0;
}
...
loom_build_t *loom_setup(int argc, char **argv) {
    ...
    char cwd[4096];
    b->root = getcwd(cwd, sizeof(cwd)) ? strdup(cwd) : strdup(".");

    for (int i = 0; i < argc; i++) {
        const char *a = argv[i];
        if (strcmp(a, "build")   == 0) b->action = "build";
        else if (strcmp(a, "run")     == 0) b->action = "run";
        else if (strcmp(a, "test")    == 0) b->action = "test";
        else if (strcmp(a, "install") == 0) b->action = "install";
        else if (strcmp(a, "query")   == 0) b->action = "query";
            ...
            const char *num = a + 2;
            if (!*num && i + 1 < argc) num = argv[++i];
            if (*num) b->jobs = atoi(num);
...
int loom_run_build(loom_build_t *b) {
    if (strcmp(b->action, "query") == 0) return do_query(b);
    ...
    for (int i = 0; i < b->n_targets; i++) {
        loom_target_t *t = b->targets[i];
        if (t->test && strcmp(b->action, "test") != 0) continue;
        if (!t->test && strcmp(b->action, "test") == 0) continue;
    ...
    if (strcmp(b->action, "run")  == 0)     return do_run(b);
    if (strcmp(b->action, "test") == 0)     return do_test(b);
    if (strcmp(b->action, "install") == 0)  return do_install(b);

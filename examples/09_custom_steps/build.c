...
void build(loom_build_t *b) {
    ...
    if (f) {
        ...
        if (tc->os == LOOM_OS_LINUX)   os = "linux";
        if (tc->os == LOOM_OS_MACOS)   os = "macos";
        if (tc->os == LOOM_OS_FREEBSD) os = "freebsd";
        if (tc->os == LOOM_OS_WINDOWS) os = "windows";

        const char *arch = "unknown";
        if (tc->arch == LOOM_ARCH_X86_64)  arch = "x86_64";
        if (tc->arch == LOOM_ARCH_AARCH64) arch = "aarch64";
        if (tc->arch == LOOM_ARCH_X86)     arch = "x86";
        if (tc->arch == LOOM_ARCH_ARM)     arch = "arm";
